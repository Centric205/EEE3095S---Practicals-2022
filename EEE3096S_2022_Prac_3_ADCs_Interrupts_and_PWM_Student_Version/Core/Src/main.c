...
/* USER CODE BEGIN PV */
char buffer[64];	// or 64
char buffer2[64];

//TO DO:
//TASK 1
//Create global variables for debouncing and delay interval
int delayTime = 500;  // delay to 500ms
int currentMillSeconds = 0;
int previousMillSeconds = 0;
uint32_t crr_value = 0;
int dutyCycle = 0;
int answer;
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  answer = pollADC(); // Starts the poll and stop the ADC and returns the most recent ADC value, since ADC
	  	  	  	  	  	  // in 12 bit mode, it will only output value between 0 and 4096.
...
void EXTI0_1_IRQHandler(void)
{
	...
	if((currentMillSeconds - previousMillSeconds) && (B1_Pin == 1))
	{
		...
		previousMillSeconds = currentMillSeconds;
	}
...
uint32_t pollADC(void){
	...
	HAL_ADC_Start(&hadc);	// ADC Start values
	HAL_ADC_PollForConversion(&hadc, 100); // ADC values obtained and stored in val
